...
class GAOptimizer(BaseOptimizer):
    ...
    def __init__(self, data_processor):
        ...
        self.generations = 50          # More generations to find improvements
    ...
    def optimize(self, generations: int = 100) -> Solution:
        ...
        try:
            for generation in range(generations):
                ...
                valid_scores = fitness_scores[fitness_scores != float('-inf')]
                avg_fitness = np.mean(valid_scores) if len(valid_scores) > 0 else float('-inf')
                ...
                if self.visualizer:
                    try:
                        # Create a copy of data to avoid threading issues
                        gen_data = {
                        ...
                        self.visualizer.update_fitness_plot(**gen_data)
                        
                        # Update route visualization with best solution
                        if best_solution:
                            self.visualizer.plot_routes(best_solution)
                ...
                # Selection
                parents = self._selection(self.population, fitness_scores.tolist())
                
                # Create new population through crossover and mutation
                new_population = []
                
                # Modified: Create one offspring at a time
                while len(new_population) < len(self.population):
                    ...
                    # Create one offspring through crossover
                ...
                # Add best solution from previous generation (elitism)
                if best_solution is not None:
                    self.population[0] = copy.deepcopy(best_solution)
            ...
            if best_solution is None:
                raise RuntimeError("Optimization failed to produce a valid solution")
        
        # Final update of visualization
        if self.visualizer and best_solution:
            ...
            self.visualizer.update_fitness_plot(**final_gen_data)
        
        print(f"\nOptimization completed. Final best fitness: {best_fitness:.2f}")
    ...
    def _build_route(self, truck_type: str, available_orders: List[str], randomize: bool = False) -> Optional[Route]:
        """Optimized route building with better parcel combinations"""
        ...
        # Pre-process orders with improved scoring
        total_weight = 0
        for order_id in available_orders:
            ...
            if weight <= capacity:
                total_weight += weight
                if total_weight > capacity * 1.2:  # Allow slightly more weight
                    break
                    
                source_loc = self._create_location(order_data['source'])
                dest_loc = self._create_location(order_data['destination'])
                
                # Enhanced scoring that favors nearby locations
                source_dist = self._calculate_distance(self.warehouse_location, source_loc)
                dest_dist = self._calculate_distance(source_loc, dest_loc)
                
                # Score based on weight utilization and distance efficiency
                weight_score = (weight / capacity) * 0.4
                distance_score = (1.0 - min(1.0, (source_dist + dest_dist) / 1000)) * 0.6
                
                # Bonus for orders that can be combined efficiently
                bonus = 0.0
                if len(valid_orders) > 0:
                    last_dest = valid_orders[-1][4]  # Last destination
                    if self._calculate_distance(last_dest, source_loc) < 500:
                        bonus = 0.2
                
                score = weight_score + distance_score + bonus
                
                if randomize:
                    score = score * 0.7 + random.random() * 0.3
        ...
        # Try to fill route with more parcels
        min_parcels = self.min_parcels_per_route if len(valid_orders) >= self.min_parcels_per_route else 2
        
        for order_id, _, weight, source_loc, dest_loc in valid_orders:
            if route_length >= self.max_route_length or remaining_capacity < weight:
            ...
            parcel = Parcel(id=mapped_id, source=source_loc, destination=dest_loc, weight=weight)
            parcels.append(parcel)
            remaining_capacity -= weight
            
            # Update locations efficiently
            if len(locations) > 1 and locations[-2] == source_loc:
                locations.insert(-1, dest_loc)
            else:
                locations.insert(-1 if locations[-1] == self.warehouse_location else len(locations), source_loc)
