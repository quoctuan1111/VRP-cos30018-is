...
class MainWindow:
    ...
    def start_optimization(self):
        ...
        self.solver.ga_optimizer.generations = ga_settings['generations']
    ...
    def _monitor_optimization(self):
        ...
        if self.is_running and self.solver and self.visualizer:
            try:
                ...
                # Update fitness plot if optimizer is available
                if current_optimizer and hasattr(current_optimizer, 'current_generation'):
                    generation = current_optimizer.current_generation
                    best_fitness = current_optimizer.best_fitness
                    fitness_scores = current_optimizer.current_population_fitness
                    
                    if fitness_scores and len(fitness_scores) > 0:  # Only update if we have valid fitness scores
                        avg_fitness = sum(fitness_scores) / len(fitness_scores)
                        
                        # Debug print for monitoring
                        print(f"Monitoring - Gen {generation}:")
                        print(f"  Best Fitness: {best_fitness:.4f}")
                        print(f"  Avg Fitness: {avg_fitness:.4f}")
                        print(f"  Population size: {len(fitness_scores)}")
                        
                        # Update visualization with current generation's data
                        if self.visualizer:
            ...
            if self.is_running:
                self.root.after(50, self._monitor_optimization)  # 50ms update interval
